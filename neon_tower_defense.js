...
const PATH_COORDS=[
  ...
  [15,10],[15,9],[15,8],[15,7],[15,6],[15,5],[15,4],[15,3],[15,2],[15,1],
  [16,1],[17,1],[18,1],[19,1],[20,1],[21,1],[22,1],[23,1],
  [23,2],[23,3],[23,4],[23,5],[23,6],[23,7],[23,8],[23,9],[23,10],[23,11],[23,12],[23,13],
  [22,13],[21,13],[20,13],[19,13],[18,13],[17,13],[16,13],
  [16,14],[16,15],[16,16],[16,17],
  [17,17],[18,17],[19,17],[20,17],[21,17],[22,17],[23,17],[24,17],[25,17],[26,17],[27,17],[28,17],[29,17],[30,17]
];
...
const TOWER_DEFS={
  basic:{name:'Базовая',cost:50,range:3,dmg:12,rate:60,color:'#7070ff',sell:25},
  sniper:{name:'Снайпер',cost:80,range:7,dmg:40,rate:120,color:'#00e5ff',sell:40},
  laser:{name:'Лазер',cost:120,range:4,dmg:8,rate:10,color:'#ff4090',sell:60},
  splash:{name:'Взрыв',cost:100,range:3.5,dmg:20,rate:90,color:'#ffb020',sell:50,splash:1.5}
};

let gold=150,hp=20,score=0,wave=0,gameState='idle';
let towers=[],enemies=[],bullets=[],particles=[];
let selectedType='basic';
let waveTimer=0,waveActive=false,enemiesLeft=0,spawnTimer=0;
...
// Game state
function resetGame(){
  gold=150;hp=20;score=0;wave=0;
  towers=[];enemies=[];bullets=[];particles=[];
  waveTimer=180;waveActive=false;enemiesLeft=0;spawnTimer=0;
  gameState='running';
...
function makeBullet(tower,enemy){
  return{x:tower.x,y:tower.y,tx:enemy,dmg:tower.dmg,speed:6,
    color:TOWER_DEFS[tower.type].color,
    splash:TOWER_DEFS[tower.type].splash||0,r:3};
}
...
function tick(){
  ...
  if(!waveActive){
    waveTimer--;
    if(waveTimer<=0){startWave();waveTimer=0;}
  }
  ...
  enemies.forEach(e=>{
    ...
    const tx=target[0]*CS+CS/2, ty=target[1]*CS+CS/2;
...
function drawTower(t){
  ...
  ctx.beginPath();ctx.rect(-8,-8,16,16);ctx.fill();
  // border glow
  ...
  if(t.type==='basic'){
    ...
    for(let i=0;i<4;i++){
      ...
      ctx.fillRect(-1,-7,2,4);ctx.restore();
    }
    ctx.beginPath();ctx.arc(0,0,3,0,Math.PI*2);ctx.fill();
  } else {
    ctx.beginPath();
    ctx.moveTo(0,-7);ctx.lineTo(6,5);ctx.lineTo(-6,5);ctx.closePath();ctx.fill();
  }
  ctx.restore();
}
...
function drawHover(){
  ...
  ctx.fillStyle=def.color;
  ctx.beginPath();
  ctx.arc(c*CS+CS/2,r*CS+CS/2,def.range*CS,0,Math.PI*2);ctx.fill();
  ctx.restore();
}
...
function render(){
  ...
  drawWaveTimer();
  // start/end markers
  ctx.fillStyle='#40ff80';ctx.font='bold 10px monospace';
...
canvas.addEventListener('click',e=>{
  ...
  const r=canvas.getBoundingClientRect();
  const mx=e.clientX-r.left, my=e.clientY-r.top;
  const c=Math.floor(mx/CS), row=Math.floor(my/CS);
  if(isPath(c,row))return;
  if(towers.find(t=>t.gc===c&&t.gr===row))return;
  const def=TOWER_DEFS[selectedType];
  if(gold<def.cost)return;
  gold-=def.cost;
  const wx=c*CS+CS/2, wy=row*CS+CS/2;
